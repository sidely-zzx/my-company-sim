...
export function FireCompensationControl({
  ...
  const baseCompensation = Math.round(employee.salaryPerDay * compensationMonths)
  ...
  return (
    <div className="grid gap-3">
        ...
        <div className="flex items-center gap-1.5 text-[13px] font-extrabold text-[#d4cbb6]">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>当前规则按日薪 * 工作月份 * 赔偿系数计算；工作月份由本公司工作天数向上折算。</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      ...
      <div className="grid gap-1.5 rounded-md border border-[#303834] bg-[#171c1b] p-3 text-[13px] text-[#d8cfbb]">
        <span>
          赔偿基数：日工资 {money(employee.salaryPerDay)} * {compensationMonths} 个月 = {money(baseCompensation)}
        </span>
