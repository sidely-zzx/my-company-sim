        ...
        <div className="mb-4 grid gap-2.5">
          ...
          <ol className="m-0 grid list-none gap-2.5 p-0">
            {pendingProjectClientEvents.map((event) => {
              ...
              return (
                <li key={event.id} className={cn('rounded-md border-l-4 bg-[rgba(12,15,15,0.72)] px-3 py-3', eventBorderToneClass[event.severity])}>
                  ...
                  <div className="grid gap-2">
                    {event.options.map((option) => (
                      <button
                        key={option.id}
                        type="button"
                        className={cn(button, 'min-h-12 justify-start whitespace-normal bg-[#1b201f] px-3 py-2 text-left text-[#efe2c8]')}
                        onClick={() => {
