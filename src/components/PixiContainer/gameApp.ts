...
const fitOfficeBackground = (sprite: Sprite, screenWidth: number, screenHeight: number) => {
  // 办公室背景是游戏世界的底图，只影响场景视觉和后续物件坐标参照，不直接改变现金流、士气、项目进度等经营属性。
  // 当容器尺寸变化时使用 cover 缩放，避免出现空边；背景的位置会影响员工、工位、装饰物等前景元素的视觉对齐。
  ...
  sprite.x = (screenWidth - width) / 2
  sprite.y = (screenHeight - height) / 2
}
...
const createGameApp = async (node: HTMLDivElement): Promise<GameAppHandle> => {
  ...
  const officeTexture = await Assets.load<Texture>(OFFICE_BACKGROUND_SRC)
  const officeBackground = new Sprite(officeTexture)
  backgroundLayer.addChild(officeBackground)

  const resizeBackground = () => {
    fitOfficeBackground(officeBackground, app.screen.width, app.screen.height)
  }

  resizeBackground()

  const resizeObserver = new ResizeObserver(resizeBackground)
