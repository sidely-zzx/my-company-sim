...
import { formatTime, percent, signedMoney } from '../game/ui';
...
export default function GamePage({
  ...
  const tutorialOverlayCoach = speedGuideSatisfied ? undefined : tutorialCoach;
  ...
  const workingEmployees = activeEmployees.filter(
    (employee) => employee.status === 'focused_work' || employee.status === 'working',
  ).length;
  ...
  const todos =
    tutorial.enabled && !tutorial.completed
      ? getTutorialTodos({
          tutorial,
          mailbox,
          laborContracts,
      ...
      : [
          {
            text: '处理未读邮件',
            meta: `${mailbox.filter((mail) => !mail.read).length} 封`,
            done: false,
          },
  ...
  const tutorialTodoDone =
    tutorial.enabled && !tutorial.completed ? todos.filter((todo) => todo.done).length : 0;
  const tutorialTodoCurrentIndex =
    tutorial.enabled && !tutorial.completed ? todos.findIndex((todo) => todo.current) : -1;
  const tutorialTodoProgress =
    tutorialTodoCurrentIndex >= 0 ? tutorialTodoCurrentIndex + 1 : tutorialTodoDone;
  const morale = Math.max(0, Math.min(100, calculateTeamMorale({ employees })));
  const efficiency = Math.round(
    activeEmployees.length > 0
      ? ((workingEmployees + idleEmployees * 0.45) / activeEmployees.length) * 100
      : 68,
  );
  ...
  return (
    <main className="grid h-full w-full grid-rows-[auto_minmax(0,1fr)_auto_auto] gap-2 bg-[linear-gradient(rgba(255,255,255,0.025)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.025)_1px,transparent_1px),#151918] bg-[length:24px_24px] p-2">
      <header
        className={cn(
          surface,
          'fixed top-0 left-0 right-0 z-30 grid min-h-20 grid-cols-[minmax(210px,1fr)_minmax(360px,1.7fr)_auto] items-stretch gap-2 px-2.5 py-2',
        )}
      >
        <div className="flex flex-nowrap items-center gap-2">
          <div className="grid h-12 w-12 place-items-center rounded-lg border border-[#7e735a] bg-[#2b2922] text-3xl font-black text-[#ead7aa] shadow-[inset_0_0_0_2px_#171a18]">
            M
          </div>
          <div>
            <h1 className="m-0 text-[23px] text-[#efe2c8]">小马科技</h1>
            <p className="m-0 text-xs font-extrabold text-[#aaa48f]">
              Day {time.day} · {formatTime(time.minuteOfDay)}
            </p>
          </div>
          <div
            className="ml-1.5 flex flex-wrap items-center gap-1.5 border-l border-[#333a37] pl-2"
            aria-label="时间速度"
          >
            <button
              type="button"
              ...
              className={cn(
                button,
                'h-[34px] min-w-[38px] bg-[#1b201f] px-2.5 text-[#d8ccb2]',
                time.speed === 0 && 'border-[#b59d65] bg-[#373226] text-[#ffe0a3]',
              )}
            >
              ||
            </button>
            <button
              ...
              className={cn(
                button,
                'h-[34px] min-w-[38px] bg-[#1b201f] px-2.5 text-[#d8ccb2]',
                time.speed === 2 && 'border-[#b59d65] bg-[#373226] text-[#ffe0a3]',
                shouldHighlightSpeedControls && cn('animate-pulse', tutorialTarget),
              )}
            >
              &gt;
            </button>
            <button
              ...
              className={cn(
                button,
                'h-[34px] min-w-[38px] bg-[#1b201f] px-2.5 text-[#d8ccb2]',
                time.speed === 6 && 'border-[#b59d65] bg-[#373226] text-[#ffe0a3]',
                shouldHighlightSpeedControls && cn('animate-pulse', tutorialTarget),
              )}
            >
              &gt;&gt;
        ...
        <div
          className="flex flex-nowrap items-center justify-between gap-1.5"
          aria-label="经营指标"
        >
          <div className="min-w-20 border-l border-[#343b38] px-2">
            <span className="block text-[12px] text-[#aea790]">现金流</span>
            <strong className={cn('mt-1 block text-[17px]', amountPositive)}>
              {money(moneyValue)}
            </strong>
          </div>
          <div className="min-w-20 border-l border-[#343b38] px-2">
            <span className="block text-[12px] text-[#aea790]">burn rate</span>
            <strong className={cn('mt-1 block text-[17px]', amountNegative)}>
              -{money(burnRate)}/天
            </strong>
          </div>
          <div className="min-w-16 border-l border-[#343b38] px-2">
            <span className="block text-[12px] text-[#aea790]">项目数</span>
            <strong className="mt-1 block text-[17px] text-[#efe2c8]">
              {activeProjectCount}/{projectContracts.length}
            </strong>
          </div>
          <div className="min-w-16 border-l border-[#343b38] px-2">
            <span className="block text-[12px] text-[#aea790]">员工数</span>
            <strong className="mt-1 block text-[17px] text-[#efe2c8]">
              {activeEmployees.length}/15
            </strong>
          </div>
          <div className="min-w-20 border-l border-[#343b38] px-2">
            <span className="block text-[12px] text-[#aea790]">满意度</span>
            <strong className="mt-1 block text-[17px] text-[#efe2c8]">
              {percent(morale / 100)}
            </strong>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          <button
            type="button"
            className={cn(button, 'h-[34px] min-w-[38px] bg-[#1b201f] px-2.5 text-[#d8ccb2]')}
            aria-label="主菜单"
            onClick={onOpenHome}
          >
            MENU
        ...
        <aside className="grid w-[230px] min-w-0 content-start gap-2">
              ...
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                ...
                {tutorialTodoTotal > 0 ? (
                  <span className={tutorialBadge}>
                    {tutorialTodoProgress}/{tutorialTodoTotal}
                  </span>
                ) : null}
              </div>
              <h2 className="mb-2 mt-1 text-xl font-black leading-6 text-[#fff8df]">
                {tutorialCoach.title}
              </h2>
              <p className="m-0 text-sm font-black leading-5 text-[#fff3cd]">
                {tutorialCoach.actionText}
              </p>
              <p className="mb-0 mt-2 text-xs font-extrabold leading-5 text-[#d8cfbb]">
                {tutorialCoach.reasonText}
              </p>
            </section>
          ) : null}
          <RunningProjectList />
              ...
              {todos.map((todo) => (
                <li
                  ...
                  className={cn(
                    ...
                    todo.current &&
                      'border border-[#ffd46a] bg-[rgba(255,212,106,0.14)] text-[#fff3cd]',
                  )}
                >
                  ...
                  <em
                    className={cn(
                      'not-italic text-[#a9a18c]',
                      todo.current && 'font-black text-[#ffcf5a]',
                      todo.done && 'text-[#6f796f]',
                    )}
                  >
                    {todo.meta}
                  </em>
          ...
          <section className={cn(surface, 'min-w-0 p-3.5')}>
            ...
            <dl className="m-0 grid">
              <div className="flex justify-between gap-3 border-b border-[#303834] py-2">
                <dt className="m-0">收入</dt>
                <dd className={cn('m-0', amountPositive)}>{money(incomeTotal)}</dd>
              </div>
              <div className="flex justify-between gap-3 border-b border-[#303834] py-2">
                <dt className="m-0">支出</dt>
                <dd className={cn('m-0', amountNegative)}>-{money(expenseTotal)}</dd>
              </div>
              <div className="flex justify-between gap-3 border-b border-[#303834] py-2">
                <dt className="m-0">净利润</dt>
                <dd className={cn('m-0', netTotal >= 0 ? amountPositive : amountNegative)}>
      ...
      <div className="fixed right-0 bottom-25 z-20">
        <aside className="grid w-[260px] min-w-0 content-start gap-2">
          ...
          {hasPendingActions && (
            ...
            >
              <span className="grid h-6 w-6 place-items-center rounded-full bg-[#bb594b] font-black text-[#fff1df]">
                !
              </span>
      ...
      <nav
        className={cn(
          surface,
          'fixed bottom-0 left-0 right-0 z-30 mx-auto grid w-[850px] grid-cols-7 gap-px',
        )}
        aria-label="模块导航"
      >
        <DockDialog
          icon="EMP"
          label="员工"
          badge={activeEmployees.length}
          highlighted={tutorialCoach?.target === 'employee'}
          hint="抓摸鱼"
          tutorialAnchor="dock-employee"
          title="员工列表"
          description="管理员工"
        >
          <EmployeePanel />
        ...
        <DockDialog
          icon="REC"
          label="招聘"
          badge={resumes.length}
          highlighted={tutorialCoach?.target === 'recruiting'}
          hint="下一步"
          tutorialAnchor="dock-recruiting"
          title="简历市场"
          description="招聘候选人"
        >
          <RecruitingPanel />
        ...
        <DockDialog
          ...
          highlighted={tutorialCoach?.target === 'project'}
          hint="下一步"
          tutorialAnchor="dock-project"
          title="项目合同"
          description="项目外包"
        >
          <ProjectPanel />
        ...
        <DockDialog
          ...
          highlighted={tutorialCoach?.target === 'event'}
          hint="处理事件"
          tutorialAnchor="dock-event"
          title="事件日志"
          description="查看事件"
        >
          <EventPanel />
        ...
        <DockDialog
          icon="CTR"
          label="合同"
          badge={laborContracts.length}
          highlighted={tutorialCoach?.target === 'labor'}
          hint="下一步"
          tutorialAnchor="dock-labor"
          title="驻场合同"
          description="人力外包合同"
        >
          <LaborPanel />
